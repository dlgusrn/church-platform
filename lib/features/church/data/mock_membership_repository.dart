...
class MockMembershipRepository implements MembershipRepository {
  ...
  Future<ChurchMembership> requestJoin({
    ...
    if (user.memberships.any((membership) => membership.church.id == churchId))
    ...
    final membership = ChurchMembership(
      id: 'membership-${DateTime.now().microsecondsSinceEpoch}',
      userId: userId,
      church: church,
    ...
    store.replaceUser(
      user.copyWith(memberships: [...user.memberships, membership]),
    );
    return membership;
  }
  ...
  @override
  Future<ChurchMembership> reject(String membershipId) async => _replace(
    membershipId,
